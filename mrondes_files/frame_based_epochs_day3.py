"""
Create frame-based Day 3 social recognition memory EEG epochs from NWB and EthoVision data.
...
the novel mouse or the familiar mouse from day 2, converts these frame-based events into 
EEG sample indices using adjusted video FPS and TTL/LED synchronization, and saves fixed-
length MNE epoch files per subject.

Original code by Mirthe Ronde; adapted for Day 3 social recognition memory analysis.
Run with: python -m mrondes_files.frame_based_epochs_day3
...
def preprocess_behavior_excel(filepath):
    """
    Load an EthoVision Day 3 Excel file and extract cup-investigation columns.

    The function identifies whether the novel stimulus mouse was placed on the
    left or right side and returns a simplified dataframe containing video frame
    numbers, time spent in the cup with the novel mouse, and time spent with the 
    familiar mouse cup.
    ...
    Returns:
    ### clean_df [pandas.DataFrame]: Dataframe with columns 'frame', 'novel cup', and
        'familiar cup'.
    """
    ...
    # Novel side is now indicated in row 40, column B (zero-based index [39, 1])
    novel_side = pd.read_excel(filepath, header=None).iloc[39, 1] # for day 2 38,1, for day 3 39,1 
    print(novel_side)
    ...
    if novel_side == 'R':
        novel_col = right_cup_col
        familiar_col = left_cup_col
    elif novel_side == 'L':
        novel_col = left_cup_col
        familiar_col = right_cup_col
    else:
        raise ValueError(f"Unexpected value in row 40, column B: {novel_side}")

    clean_df = pd.DataFrame({
        'frame': df[frame_col],
        'novel cup': df[novel_col],
        'familiar cup': df[familiar_col]
    })
...
def convert_binary_to_event_rows(df_bin):
    ...
    for behavior in ['novel cup', 'familiar cup']:
